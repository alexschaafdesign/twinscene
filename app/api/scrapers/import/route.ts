...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const { outcome, id, changes } = await upsertScrapedShow(
      {
        source,
        sourceKey,
        date,
    ...
    // `outcome` is the richer created/updated/unchanged/skipped disposition, and
    // `changes` is the field-level diff behind an "updated". Only a real write
    // changes the cached show reads — skip invalidation for skipped upserts so a
    // scrape run that mostly re-sees known shows doesn't churn the tag.
