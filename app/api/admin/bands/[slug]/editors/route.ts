...
import { listBandEditors, addBandEditor, removeBandEditor } from "@/lib/bandEditors";

export const runtime = "nodejs";
export const dynamic = "force-dynamic";

// Admin-only management of a band's editors. Every branch here re-checks
// is_admin server-side — the admin UI is just a convenience, not the gate.
...
export async function POST(request: NextRequest, { params }: { params: Promise<{ slug: string }> }) {
  ...
  const editor = await addBandEditor(band.id, email, role);
...
export async function DELETE(request: NextRequest, { params }: { params: Promise<{ slug: string }> }) {
  ...
  await removeBandEditor(band.id, userId);
