...
import type { WriterEditor } from "@/lib/writerEditors";

// Minimal admin add-by-email / remove UI for a writer profile's editors.
// Every action hits the server-gated API route. Mirrors
// MediaProEditorsManager.tsx.
export default function WriterEditorsManager({
  slug,
  initialEditors,
}: {
  slug: string;
  initialEditors: WriterEditor[];
}) {
  ...
  async function handleAdd(e: React.FormEvent) {
    ...
    setError("");
    try {
      const res = await fetch(`/api/admin/writers/${slug}/editors`, {
        method: "POST",
  ...
  return (
    <div className="mt-6">
      <ul className="flex flex-col gap-2">
        {editors.length === 0 && <li className="text-sm text-[#E8E0D0]/50">No editors assigned yet.</li>}
        {editors.map((ed) => (
          <li
            key={ed.user_id}
          ...
          >
            ...
            <button onClick={() => handleRemove(ed.user_id)} className="text-[#F5A3A3] hover:underline">
              Remove
            </button>
          </li>
        ))}
