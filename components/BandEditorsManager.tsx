...
import type { BandEditor } from "@/lib/bandEditors";

// Minimal admin add-by-email / remove UI for a band's editors. Every action
// hits the server-gated API route — this component's only job is to reflect
// what the server accepted, not to enforce anything itself.
export default function BandEditorsManager({
  slug,
  initialEditors,
}: {
  slug: string;
  initialEditors: BandEditor[];
}) {
  ...
  async function handleAdd(e: React.FormEvent) {
    ...
    setError("");
    try {
      const res = await fetch(`/api/admin/bands/${slug}/editors`, {
        method: "POST",
