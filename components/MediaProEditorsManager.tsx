...
import type { MediaProEditor } from "@/lib/mediaProEditors";

// Minimal admin add-by-email / remove UI for a media pro listing's editors.
// Every action hits the server-gated API route — this component's only job
// is to reflect what the server accepted. Mirrors BandEditorsManager.tsx.
export default function MediaProEditorsManager({
  slug,
  initialEditors,
}: {
  slug: string;
  initialEditors: MediaProEditor[];
}) {
  ...
  async function handleAdd(e: React.FormEvent) {
    ...
    setError("");
    try {
      const res = await fetch(`/api/admin/media-pros/${slug}/editors`, {
        method: "POST",
